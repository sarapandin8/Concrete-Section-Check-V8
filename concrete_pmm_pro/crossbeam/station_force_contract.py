...
CROSSBEAM.LOADS1A preserves the established member-workflow pattern: the
...
CROSSBEAM_STATION_FORCE_CONTRACT_SCHEMA = "crossbeam-station-force-import-contract-v1"
CROSSBEAM_STATION_FORCE_HANDOFF_SCHEMA = "crossbeam-station-force-analysis-handoff-v1"
...
def default_station_force_contract(
    *, effective_prestress_link: Mapping[str, Any] | None = None
    ...
    return {
        ...
        "confirmed_prestress_applied_once": False,
        "confirmed_external_fea_secondary": False,
        "confirmed_final_stage_response_basis": False,
...
def canonical_station_force_contract(
    ...
    *,
    ...
    for key in (
        ...
        result[key] = _bool(result.get(key), False)
    return result
...
def validate_station_force_contract(contract: Mapping[str, Any]) -> tuple[list[str], list[str]]:
    item = canonical_station_force_contract(contract)
    errors: list[str] = []
    warnings: list[str] = []
    ...
    if not (0.0 < loss < 60.0):
        errors.append(
            "Adopted uniform system-average total prestress loss must be greater than 0% and less than 60%."
        )
    ...
    if not item["confirmed_prestress_applied_once"]:
        errors.append("Confirm that the adopted prestress loss was applied exactly once in FEA.")
    if not item["confirmed_external_fea_secondary"]:
        errors.append("Confirm that the external portal-frame model calculated secondary prestress response.")
    if not item["confirmed_final_stage_response_basis"]:
        errors.append("Confirm that imported ULS/SLS rows are final-stage FEA responses.")
    ...
    *,
    ...
    for index, row in enumerate(canonical_rows, start=1):
        ...
        if is_sls:
            ...
            elif stage != "Final service stage":
                warnings.append(
                    f"{prefix}: {stage} is retained for compatibility; this workflow is centered on final-stage responses."
                )
...
def build_station_force_analysis_handoff(
    *,
    ...
    sls_rows: Iterable[Mapping[str, Any]] | None,
    contract: Mapping[str, Any],
    member_length_m: float,
) -> dict[str, Any]:
    ...
    canonical_sls = normalize_station_force_rows(
        sls_rows,
        contract=canonical_contract,
        response_type="SLS",
        rows_are_canonical=True,
    ...
    sls_validation = validate_station_force_rows(
        canonical_sls,
        contract=canonical_contract,
        member_length_m=member_length_m,
        response_type="SLS",
    ...
    payload = {
        ...
        "sls_rows": canonical_sls,
        "uls_validation": uls_validation.as_dict(),
        "sls_validation": sls_validation.as_dict(),
    }
    text = json.dumps(payload, sort_keys=True, separators=(",", ":"), ensure_ascii=True)
    payload["fingerprint"] = hashlib.sha256(text.encode("utf-8")).hexdigest()
    payload["ready_for_analysis"] = bool(uls_validation.ready and sls_validation.ready)
    return payload
